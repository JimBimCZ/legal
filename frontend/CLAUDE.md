...
- A development sign-in bypass keeps the local quick-start credential-free: when `GITHUB_CLIENT_ID`/`GITHUB_CLIENT_SECRET` are unset, `GET /api/auth/github` skips the GitHub round-trip entirely and issues a session for a fixed local-dev user (`github_id = 0`, which GitHub never assigns) instead. This is hard-gated so it can't reach a real deployment: `backend/app/main.py`'s `assert_safe_auth_config()` runs at startup and refuses to boot if OAuth is unconfigured while `DATABASE_URL` or `COOKIE_SECURE` is set - both of which a real deployment sets and a local run does not. The same gate independently refuses to boot when `SESSION_SECRET` is still the `config.py` default, because that default is committed to a public repo and session tokens are `itsdangerous` signatures over a bare user id - anyone could otherwise forge a session for any user, including to reach `DELETE /api/auth/me`. `COOKIE_SECURE` counts as set for any of `1`/`true`/`yes`/`on`
...
- New privacy policy at `/privacy` (`frontend/src/app/privacy/page.tsx`), linked from the dashboard, describing what is collected (GitHub id/login/email, saved document field values, chat history), who it's shared with, and how to exercise erasure. It names Cerebras as the AI inference provider reached via OpenRouter, since `document_chat.py` prefers `provider.order = ["cerebras"]` for the chat completion calls (a preference, not a pin - `order` without `allow_fallbacks: false` still lets OpenRouter route elsewhere, which is exactly why the privacy policy says "may route to a different provider")
